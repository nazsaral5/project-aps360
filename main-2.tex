...
\author{%
  ...
  https://colab.research.google.com/drive/1CPZ-N-YvvvBjDFXwUCpdjZGaCEFgUcP2?usp=sharing
\\https://github.com/nazsaral5/project_aps360.git
\\
}
\usepackage{graphicx}
\begin{document}
...
As for ethical considerations for DriveVision, it is important to note that the tool’s goal is to decipher photos of signs into five possible categories. With such a task at hand some important considerations are as follows:

\begin{enumerate}
    \item \textbf{Misclassification}: This presents as a limitation to the model, considering traffic sign recognition is critical in applications of autonomous driving and driver’s assistance technology. If a sign is misclassified, there may pose risks to the driver and/or vehicle in operation while in traffic. It is important that in evaluating my model, that there is reasonable error, to ensure its safety.
    \item \textbf{Dataset Representation and Generalization}: This is a limitation with respect to the data. Since the dataset I have chosen to use is primarily composed of European traffic signs. A model trained on one region may not generalize to another. Since I would like this model to be evaluated on Ontario traffic signs, which may differ in fonts, color schemes and layouts. To overcome this, I will add a subset of relevant MTO signs that would be found within the province to strengthen my model's use here in Ontario.
    \item \textbf{Data Collection}: As a general ethical consideration, as mentioned, I will be including a subset of my own personal data collection in the sets. All traffic sign images collected by me will be sourced from publicly available MTO data only, to ensure reliability and confidence.
\end{enumerate}
\newpage
\section*{References}
